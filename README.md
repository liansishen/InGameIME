...
在设置的“物品名称索引”页点击“建立／更新名称索引”。扫描当前物品及其公开枚举的变体，保留完整名称中的中文、英文和数字；不会自动生成语义简称。未通过模组物品枚举公开的特殊变体可能不在索引中。索引保存到 `config/ingameime-item-index-v1.json`，下次启动直接加载；更换模组、语言或资源包后请手动更新。

中文模式下，`rime_ice` 使用全拼，`double_pinyin_flypy` 使用小鹤双拼。输入音节片段即可召回名称中间的词，例如 `dslich` 匹配“动力仓”。普通候选保留 Rime 选词方式，向下越过当前 Rime 候选后可选择物品名称，空格提交。

- 输入 `+` 开始筛选：`dslich+16a+icdaxk` 查找同时包含“动力仓”、`16A`、“超大型”的完整名称，条件顺序不限。
- 空组合串开头输入半角冒号 `:` 直接检索物品，例如 `:dslich+16a`。英文模式不受影响，`v` 仍交给双拼方案处理。
- 检索中数字只用于查询，`[` / `↑` 选择上一项，`]` / `↓` 选择下一项；Page Up / Page Down 翻页，空格或回车提交完整名称。Esc 取消，没有匹配项时不会提交查询内容。
- Backspace 删除条件；删除最后一个 `+` 后恢复普通输入，`:` 入口则继续保持检索。切换输入框或关闭界面会取消组合串。

更新成功后清理此前由 InGameIME 标记生成的旧词库条目，保留用户手写内容。索引与普通 Rime 词库相互独立。
